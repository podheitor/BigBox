...
//! WebViews. Phase B exposes ONLY the read-only `scrapeChats` capability.
//! Send-side (`sendTo`) lands in Phase C / D.
//!
...
//!
//! Selectors are deliberately tolerant — the chat-service UIs are minified and
//! mutate often. Failures degrade to "name-only" rows instead of empty arrays.

/// Injected into the `whatsapp` and `whatsapp_business` WebViews.
pub const VORCARO_WHATSAPP_DRIVER: &str = r#"
(function(){
...
/// Injected into the `telegram` WebView (Web K client).
pub const VORCARO_TELEGRAM_DRIVER: &str = r#"
(function(){
  if (window.__vorcaro && window.__vorcaro.version) return;
